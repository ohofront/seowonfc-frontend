...
const fields:Record<Exclude<ResourceKey,'matches'>,Field[]>={
  news:[
    ...
    {name:'category',label:'분류',type:'select',options:[['CLUB','구단 소식'],['MATCH','경기 소식'],['MEDIA','미디어'],['NOTICE','공지사항']]},
    {name:'thumbnailUrl',label:'썸네일 이미지 주소',type:'url',placeholder:'https://example.com/image.jpg'},
  ...
  events:[
    ...
    {name:'startDate',label:'시작 일시',type:'datetime-local',required:true},
    {name:'endDate',label:'종료 일시',type:'datetime-local',required:true},
...
export default function AdminPage(){
  ...
  const close=()=>{setEditing(null);setForm({});setFormError('')};
  const save=async(event:React.FormEvent)=>{
    ...
    if(resource==='events'&&new Date(form.startDate)>new Date(form.endDate)){setFormError('종료 일시는 시작 일시보다 늦어야 합니다.');return}
    ...
    try{
      ...
      if(isEdit)await update(path,String(editing.id),payload);else await create(path,payload);
      close();
...
function AdminForm({title,fields,values,error,onChange,onSubmit,onCancel}:{title:string;fields:Field[];values:Record<string,string>;error:string;onChange:(name:string,value:string)=>void;onSubmit:(event:React.FormEvent)=>void;onCancel:()=>void}){
  return <section className="mb-10 rounded-lg border border-line bg-surface p-5 md:p-7"><h2 className="text-xl font-semibold">{title}</h2><p className="mt-2 text-sm text-muted"><span className="text-danger">*</span> 표시는 필수 입력 항목입니다.</p>
    <form className="mt-6 grid gap-5 md:grid-cols-2" onSubmit={onSubmit}>{fields.map(field=><label className={field.type==='textarea'?'md:col-span-2':''} key={field.name}><span className="label">{field.label}{field.required&&<span className="ml-1 text-danger">*</span>}</span><FormControl field={field} value={values[field.name]??''} onChange={value=>onChange(field.name,value)}/></label>)}
...
function FormControl({field,value,onChange}:{field:Field;value:string;onChange:(value:string)=>void}){
